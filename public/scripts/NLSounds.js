...
function NLSounds(soundsPath){
	...
	*/
	...
	this.setVolume = function(volume, place){
		if (this.audioWorks){
			if (volume === undefined){
				volume = 0.3;
			}
			...
			if (place !== undefined){
				this.soundList[place].volume = volume;
			}else{
				this.volume = volume;
			}
			this.refreshVolume();
	...
	this.refreshVolume = function(){
		...
		if (this.audioWorks){
			...
			for (var i in this.soundList){
				var p = this.soundList[i];
				if (this.soundList[i].volume != undefined){
					var volumeToPut = this.soundList[i].volume / 100;
				}else{
					var volumeToPut = this.volume / 100;
				}
				/*if (p.type == "sound")
					volumeToPut += this.volumeDiff / 100;*/

				if (volumeToPut < 0)
					volumeToPut = 0;
				else if (volumeToPut > 1)
					volumeToPut = 1;
